...
\begin{itemize}
\item a collection of tracks held at a remote site, is analysed by Matlab code sent to the site, 
\item the resulting descriptors are imported into a NEMA data model and transmitted to the home site, 
\item one or more machine learning framework is launched, on a specified Virtual Machine infrastructure, to predict a metadata field or transcription for each track, 
\item the predictions are also imported into the NEMA data model,
\item the results of the experiment are evaluated be MIR evaluation components provided by the NEMA project,
...
The NEMA infrastructure currently supports components that execute Java, Matlab and arbitrary binary executables and is intended to support VAMP API plugins \cite{cannam2006sonic} for music analysis in the Future. This provides an immediate path to the integration of many MIR softwares and provides a model for building hosts for further plugin environments. A diagrammatic overview of the NEMA service layers are given in figure \ref{fig_infrastructure}.
