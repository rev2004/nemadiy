 ...
 * @author kw
 */
public class WekaSMOClassifier implements SignalClassifier {

    weka.classifiers.functions.SMO theTree = null;
    String[] classnames = null;
    private String SMOOptions = "-R";
     ...
     */
    public WekaSMOClassifier() {
    }

    /**
     * Creates a new instance of TrainWekaClassifier
     *
     * @param options Options for SMOClassifier
     */
    public WekaSMOClassifier(String options) {
        SMOOptions = options;
    }
    ...
    public void train(Signal[] inputData) {
        ...
        // parse options        
        StringTokenizer stOption = new StringTokenizer(this.SMOOptions, " ");
        // System.out.println("Random option: " + SMOOptions);
        ...
        try {
            ...
            throw new RuntimeException("Failed to set SMO classifier options!", ex);
        }
        try {
            theTree.buildClassifier(trainingDataSet);
            System.out.println("WEKA: outputting SMO classifier with " + theTree.globalInfo());
        } catch (Exception ex) {
...
//                }
                ...
                try {
                    System.out.println("Exception occured when calculating distribution for a vector relating to file: " + inputSignal.getStringMetadata(Signal.PROP_FILE_LOCATION) + "!\n" + ex.getMessage());
                } catch (noMetadataException ex1) {
                    System.out.println("A noMetaDataException occured!\n" + ex1.getMessage());
                }
                ex.printStackTrace();
                return null;
            }
...
//        }
    ...
    public double[] probabilities(double[] input) {
        Instance in = new Instance(1.0, input);
        try {
            double[] probs = theTree.distributionForInstance(in);
            double sum = 0.0;
    ...
    public String getSMOOptions() {
        return SMOOptions;
    }

    public void setSMOOptions(String SMOOptions) {
        this.SMOOptions = SMOOptions;
    }
