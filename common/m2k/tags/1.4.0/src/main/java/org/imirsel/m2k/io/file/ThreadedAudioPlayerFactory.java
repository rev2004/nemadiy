 ...
 * @author Kris West
 */
public class ThreadedAudioPlayerFactory {
    /**
     ...
     */
    public static ThreadedAudioPlayer createAudioPlayer(File file){
        if (file.getAbsolutePath().toLowerCase().endsWith(".mp3")) {
            //System.out.println("Creating MP3 player.");
            return new MP3ThreadedAudioPlayer(file);
            ...
            return new WavThreadedAudioPlayer(file);
        }
        else {
            return null;
        }
