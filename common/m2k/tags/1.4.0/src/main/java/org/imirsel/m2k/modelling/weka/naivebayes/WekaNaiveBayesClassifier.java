 ...
 * @author byunggil
 */
public class WekaNaiveBayesClassifier implements SignalClassifier {

    weka.classifiers.bayes.NaiveBayes theRule = null;
    Discretize discretizer = null;
    String[] classnames = null;
    private String NaiveBayesOptions = "";
...
Use supervised discretization to process numeric attributes.
    */
     ...
     */
    public WekaNaiveBayesClassifier(String options) {
    ...
    public String getClassifierName() {
        return weka.classifiers.bayes.NaiveBayes.class.getName();
    }
    ...
    public void train(Signal[] inputData) {
        ...
        inputData = null;
        theRule = new NaiveBayes();

        // parse options
        StringTokenizer stOption = new StringTokenizer(this.NaiveBayesOptions, " ");
        String[] options;
        if ((this.NaiveBayesOptions.indexOf("-B")) == -1) { // doesn't contain "-B"
            ...
            Matcher matcher = pattern.matcher(this.NaiveBayesOptions);
        ...
        try {
            ...
            throw new RuntimeException("Failed to set NaiveBayes classifier options!", ex);
        }
		
        if (this.numBin.equals("")) { // do not discretize input data using unsupervised discretizer
            try {
                theRule.buildClassifier(trainingDataSet);
                System.out.println("WEKA: outputting NaiveBayes classifier; " + theRule.globalInfo());
            } catch (Exception ex) {
...
//        }
    ...
    public double[] probabilities(double[] input) {
        ...
        if (this.numBin.equals("")) { // do not discretize input data using unsupervised discretizer
            try {
                return theRule.distributionForInstance(anInst);
            } catch (Exception ex) {
                System.out.println("Exception occured when calculating distribution for a vector!\n" + ex.getMessage());
                ex.printStackTrace();
                return null;
    ...
    public String getNaiveBayesOptions() {
        return NaiveBayesOptions;
    }

    public void setNaiveBayesOptions(String NaiveBayesOptions) {
        this.NaiveBayesOptions = NaiveBayesOptions;
    }
