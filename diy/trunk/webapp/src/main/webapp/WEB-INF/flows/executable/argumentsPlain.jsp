...
<form:form>
	...
	<div id='envMain'><c:forEach
		items="${paramTemplate.environmentMap}" var="item">
		<div id='envParam0'><input name="variable" type="text"
			value="${item.key}" /> = <input name="vlaue" type="text"
			value="${item.value}" /><input type="button" value="-"
	...
	<div id='inputMain'><c:forEach
		items="${paramTemplate.inputParams}" var="item">
		<div id='inputParam0'><select name="input" >
			<option value="">Select</option>
			<c:forEach items="${supportedFiles}" var="file">
				<option value="${file.value}"
				<c:if test="${item==file.value}>
				selected="selected" 
				</c:if>>${file.name}</option>
			</c:forEach>
