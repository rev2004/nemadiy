...
public class JobStatusNotificationCreator implements JobStatusUpdateListener {
   ...
   private static final Logger logger = Logger
         .getLogger(JobStatusNotificationCreator.class.getName());
   ...
   public void jobStatusUpdate(Job job) {
      ...
      case UNKNOWN:
         messageBuilder.append("has been scheduled for execution: "
               + job.getSubmitTimestamp().toString() + ".");
         subjectBuilder.append("has been scheduled.");
         break;
      case SCHEDULED:
         messageBuilder.append("has been scheduled for execution: "
               + job.getScheduleTimestamp().toString() + ".");
         subjectBuilder.append("has been scheduled.");
         break;
      case SUBMITTED:
         messageBuilder.append("was submitted to the execution server: "
               + job.getSubmitTimestamp().toString() + ".");
         subjectBuilder.append("has been submitted.");
         break;
      case STARTED:
         messageBuilder.append("has started: "
               + job.getStartTimestamp().toString() + ".");
         subjectBuilder.append("has started.");
         break;
      case FINISHED:
         messageBuilder.append("has finished: "
               + job.getEndTimestamp().toString() + ".");
         subjectBuilder.append("has finished.");
         break;
      case FAILED:
         messageBuilder.append("has failed: "
               + job.getEndTimestamp().toString() + ".");
         subjectBuilder.append("has failed.");
         break;
      case ABORTED:
         messageBuilder.append("was aborted: "
               + job.getEndTimestamp().toString() + ".");
         subjectBuilder.append("was aborted.");
         break;
      }
      messageBuilder
            .append("\n\nFor more details, see http://nema.lis.uiuc.edu/diy/get/JobManager.jobDetail?id="
                  + job.getId());
      ...
      logger.fine("Preparing to persist a status change notification for job "
            + job.getId() + ".");
      notificationDao.makePersistent(notification);

      transaction.commit();
      logger.fine("Job status update notification for job " + job.getId()
